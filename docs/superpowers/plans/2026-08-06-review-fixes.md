...
describe('resolveProxyTarget', () => {
	...
	it('rejects a path that resolves to a foreign origin', () => {
		// The route captures a leading slash, so /abs//evil.example/x arrives
		// as path "/evil.example/x" — the template then builds a scheme-
		// relative URL, which new URL() happily resolves to the foreign host.
