...
async function proxy(request: Request, params: { path: string }, method: string): Promise<Response> {
	...
	try {
		...
		return new Response(JSON.stringify({ error: 'Proxy error', target: target.toString() }), { status: 502 });
	}
}
...
export const DELETE: RequestHandler = async ({ request, params }) => proxy(request, params, 'DELETE');
