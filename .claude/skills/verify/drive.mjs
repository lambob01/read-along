...
await page.evaluate(() => {
	localStorage.setItem(
		'reader-connection',
		JSON.stringify({ url: 'http://localhost:13999', token: 'test-token', connected: true })
	);
	localStorage.setItem(
		'reader-settings',
		JSON.stringify({
...
const canPlay = await page.evaluate(async () => {
	const a =
		...
		[...document.querySelectorAll('*')].find((e) => e.tagName === 'AUDIO');
	return { audioInDom: !!a };
});
...
for (let i = 0; i < 14; i++) {
	...
	const s = await page.evaluate(() => {
		...
		return {
			t: el ? Number(el.value).toFixed(2) : null,
