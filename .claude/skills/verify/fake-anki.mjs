  ...
  req.on('end', () => {
    ...
    try {
      switch (action) {
        ...
        case 'deckNames': result = ['Default', 'Mining']; break;
        case 'modelNames': result = Object.keys(models); break;
        case 'modelFieldNames': result = models[params.modelName] || []; break;
        case 'findNotes': result = [...notes.keys()]; break;
        case 'notesInfo':
          result = params.notes.map((id) => {
            const n = notes.get(id);
            return { noteId: id, modelName: n.modelName, tags: n.tags,
        ...
        case 'storeMediaFile': {
          ...
          console.log('  stored media:', params.filename, Buffer.from(params.data, 'base64').length, 'bytes');
